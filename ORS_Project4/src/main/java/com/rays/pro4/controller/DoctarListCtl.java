...
@WebServlet(name = "DoctarListCtl", urlPatterns = { "/ctl/DoctarListCtl" })

public class DoctarListCtl extends BaseCtl {
	...
	protected BaseBean populateBean(HttpServletRequest request) {
		...
		bean.setExperties(DataUtility.getString(request.getParameter("Expp")));
		//bean.setId(DataUtility.getLong(request.getParameter("Expp")));
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (OP_SEARCH.equalsIgnoreCase(op)) {
			...
			if (ids != null && ids.length > 0) {
				DoctarBean deletebean = new DoctarBean();
				for (String id : ids) {
	...
	protected String getView() {
		...
		return ORSView.DOCTAR_LIST_VIEW;
	}
